...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then
	...
	printf "Usage:
		...
		-3rd argument	Gene abundance file produced by each individual StringTie assembly\n\n"
	exit 1
fi
...
#	duplicate_features=$(tail -n +2 $line | cut -f1,10 | cut -f1 |sort | uniq -cd | wc -l)
	...
	#ISOFORM ANALYSIS#
	unique_isoforms=$(cat $line | grep -v "#" | grep -w "transcript" | cut -f9 | cut -f2,3 -d ";" | sort | uniq | wc -l)
