...
def processVCF(invcf,remm,out):
    ...
    vcf_data.add_info_to_header({'ID': 'TraP', 'Description': 'Transcript-inferred Pathogenicity score',
                                 'Type':'String', 'Number': '.'})
...
def main():
    parser = argparse.ArgumentParser(description='Script to append TrAP scores into a VCF')
    parser.add_argument(dest='vcf', help='Path to the vcf file')
    parser.add_argument(dest='TraP', help='Path to the TraP scores file. A tabix indexed file is required')
