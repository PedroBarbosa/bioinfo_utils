...
def process_voila_files(files, threshold, probability_threshold, groups=None):
    pos_gens_GO, neg_genes_GO = defaultdict(set), defaultdict(set)
    per_file_ranks, final_ranks_to_GSEA = defaultdict(list), {}
    ...
    for f in files:
        ...
        with open(f, 'r') as infile:
            ...
            for line in infile:
                ...
                per_file_ranks[gid.split('.')[0]].append((prob_max, max_delta))
                if abs(float(max_delta)) < threshold or float(prob_max) < probability_threshold:
                    neg_genes_GO[groups[f][0]].add(gid.split('.')[0])
                    continue
        ...
        final_ranks_to_GSEA[groups[f][0]] = per_file_ranks
    return lsv, header, [pos_gens_GO, neg_genes_GO, final_ranks_to_GSEA]


def write_output(lsvs, header, outbasename, tobed, enrichment_data, groups=None):
    d = defaultdict(list)
    strand_map = {"-": "minus", "+": "plus"}
    for file in enumerate(enrichment_data[0])
